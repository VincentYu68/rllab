...
def estimate_splitability(algo, policy, total_paths, init_params, pol_weights, segment_num):
    path_count = [0] * segment_num
    for i in range(len(total_paths)):
        for path in total_paths[i]:
            path_count[path['env_infos']['dyn_model_id'][-1]] += 1.0
    ...
    for i in range(len(total_paths)):
        ...
        for path in total_paths[i]:
            path_collections[path['env_infos']['dyn_model_id'][-1]].append(path)
    ...
    return np.sum(split_metrics), split_counts
...
if __name__ == '__main__':
    ...
    policy = joblib.load(
        'data/local/experiment/cartpole_sd11_init_10k/policy_20.pkl')
    ...
    if not load_metric_from_file:
        ...
        splitability, split_counts = estimate_splitability(algo, policy, all_paths, policy_params, pol_weights, segmentation_num)
        joblib.dump(split_counts, 'data/trained/gradient_temp/' + folder_name + '/split_metric.pkl', compress=True)
    else:
